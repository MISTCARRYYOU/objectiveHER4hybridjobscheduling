...
class Trainer(object):
    ...
    def create_agent_to_agent_group_dictionary(self):
        ...
        agent_to_agent_group_dictionary = {
            ...
            "TD3-HER": "Actor_Critic_Agents",
            "OTDPG": "Actor_Critic_Agents",
            "OTDPGN1": "Actor_Critic_Agents",
            "OTDPGN2": "Actor_Critic_Agents",
            "A2C": "Actor_Critic_Agents",
            "A3C": "Actor_Critic_Agents",
            "AC": "Actor_Critic_Agents",
            "h-DQN": "h_DQN",
            "SNN-HRL": "SNN_HRL",
            "HIRO": "HIRO",
    ...
    def run_games_for_agents(self):
        ...
        for agent_number, agent_class in enumerate(self.agents):
            ...
            # self.config.load_path = './my_data_and_graph/models/' +agent_name + '/' + self.config.environment.env_name + '-' + agent_name + '-' + str(self.config.load_path_epix) + '.pt'
            self.config.load_path = './my_data_and_graph/models/' +agent_name + '/CEDCS__500_seed1-' + agent_name + '-' + str(self.config.load_path_epix) + '.pt'

            if 'seed2' in self.config.load_path or 'seed3' in self.config.load_path:
                self.config.load_path = self.config.load_path.replace('seed2', 'seed1')
                self.config.load_path = self.config.load_path.replace('seed3', 'seed1')
                print('Attention ! !!!!!!!!\n!!!!!!!!!\n!!!!!!!!!! \n !!!!!!!!!!!!! Generation is tested without the original env trained models !!!!!!!!!!!!!!!!')
            with open('./my_data_and_graph/' + self.config.environment.env_name + 'logs.txt', 'a') as f:
                print(agent_name, ': --------------------------------', file=f)

            self.run_games_for_agent(agent_number + 1, agent_class)
            t2 = time.time()
            with open('./my_data_and_graph/' + self.config.environment.env_name + 'times.txt', 'a') as f:  # record the time
                print(agent_name, (t2-t1) / self.config.num_episodes_to_run, file=f)
