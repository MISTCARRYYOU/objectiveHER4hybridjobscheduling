...
from agent_DRL.agents.Base_Agent import Base_Agent
from agent_DRL.utilities.data_structures.Replay_Buffer import Replay_Buffer
# from agent_DRL.exploration_strategies.OU_Noise_Exploration import OU_Noise_Exploration
from agent_DRL.exploration_strategies.OTD_exploration_strategy import OTD_Exploration


class DDPG_OTD(Base_Agent):
    """A DDPG Agent for improvement"""
    agent_name = "DDPG-OTD"

    def __init__(self, config, is_her=False):
        # self.agent_name = "DDPG"
        self.is_state_dict = is_her
        ...
        if config.is_load:  # if load the model
        ...
        self.exploration_strategy = OTD_Exploration(config)
    ...
    def step(self):
        ...
        while not self.done:
            ...
            if self.time_for_critic_and_actor_to_learn() and self.config.is_train:
                # learning starts
                ...
                # save the model
                # if self.episode_number % self.config.save_fre == 0:
                #     self.save_policy()
    ...
    def pick_action(self, state=None):
        ...
        if self.config.is_train:  # no train no exploration zezeze
            action = self.exploration_strategy.perturb_action_for_exploration_purposes({"action": action, 'rate':(1-self.episode_number/self.config.num_episodes_to_run)})
    ...
    def locally_save_policy(self, obj):
        torch.save(self.actor_local.state_dict(), './my_data_and_graph/models/' + self.config.environment.env_name + '-' +
                   self.agent_name + '-' + str(self.episode_number) + '-' + str(obj) + '.pt')

    def load_model(self, modelpath):
        self.actor_local.load_state_dict(torch.load(modelpath))
        print('Pretrained model of', modelpath, ' has been loaded !!')
