...
class Base_Agent(object):
    ...
    def get_state_size(self):
        ...
        random_state = self.environment.reset(self.is_state_dict)
    ...
    def reset_game(self):
        ...
        self.state = self.environment.reset(self.is_state_dict)
    ...
    def run_n_episodes(self, num_episodes=None, show_whether_achieved_goal=True, save_and_print_results=True):
        ...
        while self.episode_number < num_episodes:
            ...
            self.step()
            temp_t = time.time()
            temp_obj = self.environment.obtain_obj()
            self.epi_run_time = temp_t - temp_t0
            if save_and_print_results: self.save_and_print_result()
            if self.episode_number % self.config.save_fre == 0 and self.episode_number != 0:
                self.locally_save_policy(temp_obj)
    ...
    def conduct_action(self, action):
        ...
        # print('next_state:', self.next_state, 'action:',action , 'reward:', self.reward)
        # array dim=1 ; float ; boolean
        if self.episode_number % self.config.record_interval == 0:
            with open('./my_data_and_graph/' + self.environment.env_name + '_trans_' + self.agent_name + '.txt', 'a') as f:
                print('done/reward/state: ', self.done, self.reward, self.next_state.tolist(), file=f)
    ...
    def save_result(self):
        ...
        # print(self.environment.all_states.get_current_cost(), 6666666)  # tensor([1.3190]) 6666666
    ...
    def print_rolling_result(self):
        ...
        text = """"\r Episode {0}, Episode reward: {1: .2f}"""
        if self.agent_name[:3] != 'SAC':
            sys.stdout.write(text.format(len(self.game_full_episode_scores), self.game_full_episode_scores[-1]))
            sys.stdout.flush()
    ...
    def create_NN(self, input_dim, output_dim, key_to_use=None, override_seed=None, hyperparameters=None):
        ...
        default_hyperparameter_choices = {"output_activation": 'relu', "hidden_activations": "relu", "dropout": 0.0,
