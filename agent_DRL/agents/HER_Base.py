...
from agent_DRL.utilities.Utility_Functions import abstract
...
class HER_Base(object):
    ...
    def reset_game(self):
        ...
        self.state_dict = self.environment.reset(True)
    ...
    def conduct_action_in_changeable_goal_envs(self, action):
        ...
        self.next_state_dict, self.reward, self.done, _ = self.environment.step(action, is_state_dict=True)
        if self.episode_number % self.config.record_interval == 0:
            with open('./my_data_and_graph/' + self.environment.env_name + '_trans_' + self.agent_name + '.txt',
                      'a') as f:
                print('done/reward/obs/goal: ', self.done, self.reward, self.next_state_dict['observation'].tolist(), self.next_state_dict['desired_goal'].tolist(),  file=f)
