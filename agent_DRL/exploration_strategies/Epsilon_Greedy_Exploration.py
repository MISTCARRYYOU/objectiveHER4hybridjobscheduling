...
class Epsilon_Greedy_Exploration(Base_Exploration_Strategy):
    ...
    def perturb_action_for_exploration_purposes(self, action_info):
        ...
        avail_actions = action_info['avail_actions']
        ...
        if self.config.evaluate:  # 如果是评估的话就不探索了
            action_values = action_values[0]
            assert action_values.shape[0] == avail_actions.shape[0]
            action_values[avail_actions == True] = -9999
            res = torch.argmax(action_values).item()
            ...
            if (random.random() > epsilon or turn_off_exploration) and (episode_number >= self.random_episodes_to_run):
                ...
                avail_actions = (avail_actions == False).nonzero().squeeze()
                if len(avail_actions.shape) == 0:
                    res = avail_actions.item()
                else:
                    res = np.random.choice(avail_actions, 1, replace=False)[0]
        return res
