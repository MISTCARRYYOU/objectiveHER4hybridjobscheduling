...
# 基于动作mask生成分布
def create_actor_distribution_with_mask(action_types, actor_output, action_size, action_mask):
    """Creates a distribution that the actor can then use to randomly draw actions"""
    if action_types == "DISCRETE":
        assert actor_output.size()[1] == action_size, "Actor output the wrong size"
        action_mask = action_mask[None, :]
        # print(000, actor_output)
        actor_output[action_mask == True] = 0
        # print(111,action_mask)
        # print(222,actor_output)
        if actor_output.sum() == 0: # 如果mask之后全都是0的话
            actor_output[action_mask == False] = 1
        action_distribution = Categorical(actor_output)  # this creates a distribution to sample from
